...
namespace BLL_OnlineStore.Services
{
    ...
    //            await _personRepository.SaveAsync(); // تأكد من طريقة الحفظ في IRepository
    ...
    //            // 3. إنشاء Customer record
    //            var customer = new Customer
    //            {
    //                PersonId = person.PersonId,
    //                // ... add other default customer properties
    //            };

    //            await _customerRepository.AddAsync(customer);
    //            await _customerRepository.SaveAsync();

    //            // 4. إضافة دور Customer
    //            await _userManager.AddToRoleAsync(user, "Customer");

    //            // 5. Commit Transaction
    //            await transaction.CommitAsync();

    //            // 6. إنشاء JWT وبناء الـ AuthModel
    //            var jwtToken = await CreateJwtToken(user);
    //            return await BuildAuthModel(user, jwtToken);
    //        }
    //        catch (Exception ex)
    ...
    //            IsAuthenticated = true,
    //            Token = jwtToken,
    //            ExpirOn = DateTime.UtcNow.AddDays(_jwtSettings.DurationInDays),
    //            Roles = roles.ToList(),
